...
\tt{Cilk} is a general-purpose language designed for multithreaded parallel
computing. The \tt{C++} incarnation is called \tt{Cilk+}. The biggest
principle behind the design of the Cilk language is that the programmer should
be responsible for \emph{exposing} the parallelism, identifying elements that
can safely be executed in parallel; it should then be left to the run-time
environment, particularly the scheduler, to decide during execution how to
actually divide the work between processors.

The crucial thing is that two keywords are all that are needed to start
using the parallel features of \tt{Cilk+}:
\begin{itemize}
...
\EndProcedure
\end{algorithmic}
We just tells \tt{Cilk+} that the subtrees rooted at various children can be
...
\end{algorithmic}
then we face the problem of to core incrementing the same variable at the same
time. This is called a data race and leads to nondeterministic wrong
result. Cilk provide the notion of \emph{reducer} which are variables local to
each thread which are gathered (here added) when a thread is finishing its
job.
