...
\begin{abstract}
  ...
  architecture of modern computer allowing very large optimizations.
\end{abstract}
...
tree~$\mathcal{T}$ and allow \Florent{large} code optimization essentially based on use of
vectorial instructions and parallelization. \Florent{The goal of the paper is twofold:
...
\begin{algorithm}
\caption{Returns the root of $\mathcal{T}$ for an exploration up to genus \tt{G}}\label{A:Root}
\begin{algorithmic}
\small
\Function{Root}{\tt{G}}
...
\begin{algorithm}
\caption{\Cilk version of Algorithm~\ref{A:ExpRec}}
\begin{algorithmic}
\Procedure{ExploreRec}{\tt{S}, \tt{G}}
	\If{$g(\tt{S})< \tt{G}$}
		\For{\tt{x} from $c(\tt{S})$ to $c(\tt{S})+m(\tt{S})$}
