...
algorithms, and second to present the optimization technique which allows for
those kinds of algorithms to get speedups by factors of hundreds and even
thousand. We claim that these technique are fairly general for those kinds of
...
Running the \Cilk version of our optimized algorithm  we have explored the tree of numerical semigroup upto genus $67$
The computation are done on $64$ cores \REM{Ajouter descriptif de la machine + temps mis}.
The values of $n_g$ for $g\leq 67$ are : 
\begin{center}
...
It should be mentioned that the hyper threading of the CPU has not been deactivated : the CPU clock is higher using one thread than  two and so on.

\textbf{Acknowledgment.} We would like to thank Shalom Eliahou for his presentation of the tree of numerical semigroups and for the interesting discussions on the subject.
