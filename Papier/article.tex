...
The cost to pay is that, if we want to explore the tree deepero, we must restart from the root.
...
Since by \eqref{E:c} and \eqref{E:m} we have $x\leq 3G$ together with $m(S)\leq G+1$, each line from \tt{2} to \tt{8} is done in time $O(\log(G))$.
